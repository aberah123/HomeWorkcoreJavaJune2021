...
/* Two keywords(extends and implements)are used for the inheritance in Java for abstract class.
 * yes,An abstract class can inherit only one abstract class or only one regular class by extends keyword but can not inherit interface by extends keywords.
 * implements keyword can inherit more than one interface.
 * One extends keywords are used for the inheritance for Interface. 
 * An Interface cannot inherit a regular class or abstract class. only one inheritances is
